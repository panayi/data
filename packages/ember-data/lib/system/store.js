...
// Adds the ability to seed some `hasMany` associations in a newly created record
// similar to how Rails seed works. Specify a seeds object as App.Post.SEEDS = {}
// and this extension will handle the creation of associated records. 
DS.Store.reopen({
  createRecord: function(type, properties, transaction) {
    var record = this._super(type, properties, transaction);
    if (!type.SEEDS) return record;

    var seeds = type.SEEDS;
    properties = properties || {},
    store = this;

    // Loop through the hasMany associations and check if Seed data
    // for that association is present.
    var associationType, childRecords, counterPartName, r;
    for (associationName in seeds) {
      associationType = DS.inverseTypeFor(type, associationName);
      if (!!associationType) {
        childRecords = [];

        counterPartName = DS.inverseNameFor(associationType, type, 'belongsTo');
        seeds[associationName].forEach(function(hash) {
          r = associationType.createRecord(hash);
          r.set(counterPartName, record);
          childRecords.pushObject(r);
        });

        properties[associationName] = properties[associationName] || [];
        properties[associationName].addObjects(childRecords);
      }
    }
